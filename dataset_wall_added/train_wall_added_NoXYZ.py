...
class PointCloudDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        features = distances.astype(np.float32) # change to uncommented for only NN-distance 
        # features = np.concatenate([robot_scan, distances], axis=1).astype(np.float32)  # Shape: (N, 4)
...
class MismatchDetectionNet(nn.Module):
    def __init__(self, dropout_rate=0.3):
        ...
        self.conv1 = nn.Conv1d(1, 64, 1) # change to uncommented for only NN-distance
...
npy_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_added\clean_and_occluded\npy_files_no_augmented" # change this per run
labels_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_added\clean_and_occluded\labels_no_augmented" # change this per run
...
if TRAIN_MODEL:
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 50 == 0:
            model_save_path = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_added\mismatch_detection_model_per_point_wall_added_clean_and_occluded_1_no_augmented_no_xyz_epoch_{}.pth".format(epoch + 1) # change this per run
            torch.save(model.state_dict(), model_save_path)
            print(f"[CHECKPOINT] Saved model to {model_save_path}")

            # Save logs up to current epoch
            log_dir = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_added" # change per run
    ...
    torch.save(model.state_dict(), r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_added\mismatch_detection_model_per_point_wall_added_clean_and_occluded_1_no_augmented_no_xyz.pth") # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_added\train_losses_clean_and_occluded_1_no_augmented_no_xyz.npy", np.array(train_losses)) # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_added\val_losses_clean_and_occluded_1_no_augmented_no_xyz.npy", np.array(val_losses)) # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_added\val_precisions_clean_and_occluded_1_no_augmented_no_xyz.npy", np.array(val_precisions)) # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_added\val_recalls_clean_and_occluded_1_no_augmented_no_xyz.npy", np.array(val_recalls)) # change this per run
