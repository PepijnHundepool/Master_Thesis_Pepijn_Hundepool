...
class PointCloudDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        features = distances.astype(np.float32)
...
npy_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_removed\clean_and_occluded\npy_files_filtered_no_augmented" # change this per run
labels_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_removed\clean_and_occluded\labels_npy_filtered_no_augmented" # change this per run
...
if TRAIN_MODEL:
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 50 == 0:
            model_save_path = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_removed\mismatch_detection_model_per_point_wall_removed_clean_and_occluded_2_no_augmented_no_xyz_epoch_{}.pth".format(epoch + 1) # change this per run
            torch.save(model.state_dict(), model_save_path)
            print(f"[CHECKPOINT] Saved model to {model_save_path}")

            # Save logs up to current epoch
            log_dir = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_removed" # change this per run
    ...
    torch.save(model.state_dict(), r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_removed\mismatch_detection_model_per_point_wall_removed_clean_and_occluded_2_no_augmented_no_xyz.pth") # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_removed\train_losses_clean_and_occluded_2_no_augmented_no_xyz.npy", np.array(train_losses)) # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_removed\val_losses_clean_and_occluded_2_no_augmented_no_xyz.npy", np.array(val_losses)) # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_removed\val_precisions_oclean_and_ccluded_2_no_augmented_no_xyz.npy", np.array(val_precisions)) # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_wall_removed\val_recalls_clean_and_occluded_2_no_augmented_no_xyz.npy", np.array(val_recalls)) # change this per run
