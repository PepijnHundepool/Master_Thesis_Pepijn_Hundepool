...
TRAIN_MODEL = False  # change to False to skip training and run only testing
threshold = 0.6
...
class PointCloudDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        robot_scan_file = scan_file.replace("dt_scan", "robot_scan")

        dt_scan = np.load(os.path.join(self.npy_folder, scan_file))
        robot_scan = np.load(os.path.join(self.npy_folder, robot_scan_file))

        # kdtree = KDTree(dt_scan[:, :3])
        # distances, _ = kdtree.query(robot_scan[:, :3], k=1)
        # features = distances  # Already (N, 1)

        nbrs = NearestNeighbors(n_neighbors=1, algorithm='auto').fit(robot_scan)
        distances, _ = nbrs.kneighbors(dt_scan)
        features = distances.astype(np.float32)  # Shape: (N, 1)
        # features = np.concatenate([dt_scan, distances], axis=1).astype(np.float32)  # Shape: (N, 4)
        ...
        return (
            torch.tensor(dt_scan, dtype=torch.float32),
...
class MismatchDetectionNet(nn.Module):
    ...
    def forward(self, x):
        """
        x shape: [B, N, 1] → 1 features per point (distance)
        """
        B, N, _ = x.shape
...
npy_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_pillar_removed\clean_and_occluded\npy_files_filtered_no_augmented" # change this per run
labels_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_pillar_removed\clean_and_occluded\labels_npy_filtered_no_augmented" # change this per run
...
if TRAIN_MODEL:
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 50 == 0:
            model_save_path = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_pillar_removed\mismatch_detection_model_per_point_pillar_removed_clean_and_occluded_3_no_augmented_no_xyz_epoch_{}.pth".format(epoch + 1) # change this per run
            torch.save(model.state_dict(), model_save_path)
            print(f"[CHECKPOINT] Saved model to {model_save_path}")

            # Save logs up to current epoch
            log_dir = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_pillar_removed" # change this per run
    ...
    torch.save(model.state_dict(), r"D:\Graduation Project\Pointclouds\total\datasets\dataset_pillar_removed\mismatch_detection_model_per_point_pillar_removed_clean_and_occluded_3_no_augmented_no_xyz.pth") # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_pillar_removed\train_losses_clean_and_occluded_3_no_augmented_no_xyz.npy", np.array(train_losses)) # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_pillar_removed\val_losses_clean_and_occluded_3_no_augmented_no_xyz.npy", np.array(val_losses)) # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_pillar_removed\val_precisions_clean_and_occluded_3_no_augmented_no_xyz.npy", np.array(val_precisions)) # change this per run
    np.save(r"D:\Graduation Project\Pointclouds\total\datasets\dataset_pillar_removed\val_recalls_clean_and_occluded_3_no_augmented_no_xyz.npy", np.array(val_recalls)) # change this per run
