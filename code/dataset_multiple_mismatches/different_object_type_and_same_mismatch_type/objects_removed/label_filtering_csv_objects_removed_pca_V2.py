...
def filter_labels_pca_clustering(input_dir, output_dir, k_neighbors=50,
    ...
    for file in input_dir.glob("*.csv"):
        ...
        for idx in range(len(points)):
            ...
            # PCA XY elongation ratio (optional)
            pca_xy = PCA(n_components=2)
            pca_xy.fit(cluster_points[:, :2])
            eigvals = pca_xy.explained_variance_
            ratio = eigvals[0] / (eigvals[1] + 1e-6)
            print(f"[DEBUG] PCA XY elongation ratio: {ratio:.2f}")
            # if ratio > 10.0:
            #     continue  # very elongated cluster in XY → likely wall or linear shadow
...
if __name__ == "__main__":
    # geometry_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_pillar_removed\occluded\labels_csv_improved" # change this per run
    # output_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_pillar_removed\occluded\labels_csv_filtered_improved" # change this per run

    # geometry_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_real_world\pillar_removed\occluded\labels_csv" # change this per run
    # output_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_real_world\pillar_removed\occluded\labels_csv_filtered" # change this per run

    geometry_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_multiple_mismatches\different_object_type_and_same_mismatch_type\objects_removed\occluded\labels_csv"
    output_folder = r"D:\Graduation Project\Pointclouds\total\datasets\dataset_multiple_mismatches\different_object_type_and_same_mismatch_type\objects_removed\occluded\labels_csv_filtered"
    
    filter_labels_pca_clustering(geometry_folder, output_folder,
                                 k_neighbors=50,
                                 verticality_thresh=0.99,
                                 max_neighbor_distance=0.142,
                                 min_cluster_size=50,
                                 min_height=2.4)
